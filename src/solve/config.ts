// Загрузчик config/solve.md (фаза 2.4a). Тот же паттерн, что src/investigate/config.ts:
// один json-блок в markdown, парсится при каждом вызове solveDiagnosis()
// (1×/diagnosis с verdict='code', не критично).
//
// Меняешь модель/тайм-аут/whitelist — правишь config/solve.md, не код.
...
export interface SolveConfig {
  ...
  // Fan-out параметры (фаза 2.4b). См. config/solve.md секция «Поля».
  concurrency: number;
...
const CONFIG_PATH = 'config/solve.md';

export async function loadSolveConfig(rootDir: string = process.cwd()): Promise<SolveConfig> {
...
function extractJsonBlock(markdown: string, path: string): SolveJson {
  const match = markdown.match(/```json\s*\n([\s\S]*?)\n```/);
  if (!match || match[1] === undefined) {
    throw new Error(
      `${path}: не найден json-блок \`\`\`json ... \`\`\`. Настройки решателя должны лежать в первом json-блоке файла.`,
    );
  }
  try {
    return JSON.parse(match[1]) as SolveJson;
  } catch (err) {
...
function validateShape(json: SolveJson, path: string): void {
