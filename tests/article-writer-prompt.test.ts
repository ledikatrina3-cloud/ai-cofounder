...
describe('article-writer prompt audience framing', () => {
  ...
  const prompt = readFileSync(resolve(process.cwd(), 'agents/article-writer/prompt.md'), 'utf8');
  const examplePrompt = readFileSync(resolve(process.cwd(), 'examples/agents/article-writer/prompt.md'), 'utf8');
  const rules = readFileSync(resolve(process.cwd(), 'agents/article-writer/rules.md'), 'utf8');
  ...
  it('does not force plastic viral-gate tokens into the lead', () => {
    for (const promptVariant of promptVariants) {
      ...
      expect(compactPromptVariant).toContain('число допускается только если оно подтверждает смысл');
