...
1. **Сначала прочитай `org/cover-design-brief.md`** и используй его как источник
   правды по палитре, композиции и запретам. Если `org/design-system.md`
   существует, учитывай его как общий visual law проекта.
2. **Выбери template** по типу статьи. Доступные:
...
3. **Запусти скрипт:**
   ...
   ```
4. **Если sharp установлен** — скрипт автоматически конвертит SVG → PNG (1200×630
   под Open Graph). Иначе только SVG.

## Ограничения

- Title до 80 символов рекомендовано. Длинные русские title надо переносить на
  3-4 короткие строки, сохраняя безопасный правый отступ в PNG-превью.
