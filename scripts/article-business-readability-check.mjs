...
if (!candidateArg) {
  console.error('Usage: node scripts/article-business-readability-check.mjs content/<slug>.md [--lead-chars 1500] [--json]');
  process.exit(2);
}
...
function firstParagraph(markdown) {
  return contentAfterH1(markdown)
    .split(/\r?\n\s*\r?\n/)
...
function findToolAgencyPhrases(text) {
  ...
  const pattern = new RegExp(`(?:^|[^а-яa-z0-9])(${subjectPattern})(?:\\s+\\S+){0,3}\\s+(${verbPattern})(?=$|[^а-яa-z0-9])`, 'giu');
  const matches = [];
  let match;
...
function articleInfo(filePath) {
  ...
  const technicalHeadingCount = normalizedHeadings.filter((heading) => countPatterns(heading, TECHNICAL_PATTERNS) > 0).length;
  const businessHeadingCount = normalizedHeadings.filter((heading) => countPatterns(heading, BUSINESS_PATTERNS) > 0).length;
  ...
  const metrics = {
    ...
    article: {
      ...
      technicalTermsPerThousandWords: totalWords > 0 ? Math.round((totalTechnicalTerms / totalWords) * 1000) : 0,
      toolAgencyPhrases,
    },
  };
  ...
  if (metrics.firstParagraph.startsAsWorkshop) {
    issues.push({
      ...
      message: 'The opening starts with a process instruction instead of a business scene or consequence.',
    });
  }
  ...
  if (metrics.firstParagraph.toolBeforePain) {
    issues.push({
      ...
      message: 'The first paragraph introduces automation/tool/process terms before making the business pain clear.',
    });
  }
  ...
  if (
    ...
    issues.push({
      ...
      message: 'The first 1500 characters need more customers, money/time/sales consequences, and recognizable human scene.',
    });
  }
  ...
  if (
    ...
    issues.push({
      ...
      message: 'Too many H2 headings are built around statuses, rules, exceptions, pilots, queues, or similar process terms.',
    });
  }
  ...
  if (metrics.article.toolAgencyPhrases.length > 0) {
    issues.push({
      ...
      message: 'Tools should not be described with human-skill verbs like "умеет"; rewrite as "в CRM можно...", "настройка позволяет...", or name the human/process action.',
