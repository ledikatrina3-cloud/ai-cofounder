...
if (!candidateArg) {
  console.error(
    'Usage: node scripts/article-diversity-check.mjs content/<slug>.md [--content-dir content] [--recent 5] [--json]',
  );
  process.exit(2);
}
...
function articleInfo(filePath) {
  ...
  const categories = {
    one_object_drill:
      /(провед|передайте|опишите|разберите|возьмите|начните)\s+(1|один|одну|одно|обычн|последн).{0,90}(заказ|счет|счёт|товар|заявк|процесс|результат|документ|согласован|строк|объект)/.test(
        firstHalf,
      ) ||
      /(провед|передайте|опишите|разберите|возьмите|начните).{0,60}(через|по).{0,40}(шаг|передач|событ|пол)/.test(
        headingText,
      ),
    queue_status_core: /(очеред|статус|ждет|ждут|завис|следующий шаг|в работе|передач)/.test(
      headingText,
    ),
    exception_core: /(исключ|ошиб|сбой|расхожд|не хватает|непол|просроч|проблем)/.test(headingText),
    rule_owner_core: /(правил|владел|ответствен|кто должен|кто решает|кому|у кого)/.test(
      headingText,
    ),
    pilot_readiness_end: /(пилот|автоматизац|когда пора|рано|готов|специалист|система)/.test(
      headingText,
    ),
    table_card_artifact:
      /(таблиц|карточк|карта|бриф|письм|вопрос).{0,90}(пол|передач|событ|шаг|строк|руководител|специалист)/.test(
        text,
      ),
    action_final: /(пора|останов|письм|бриф|вопрос|маленьк|специалист|когда|готов|шаг)/.test(
      finalH2,
    ),
  };
...
const comparisons = recent.map((article) => {
  const overlap = candidate.categoryNames.filter((category) =>
    article.categoryNames.includes(category),
  );
  return {
    path: article.path,
    title: article.title,
...
if (
  ...
  issues.push({
    ...
    message:
      'Recent posts already use the "run one object through N steps/transfers" practical block.',
  });
}
